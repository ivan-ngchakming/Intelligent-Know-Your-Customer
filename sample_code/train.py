...
face_cascade = cv2.CascadeClassifier(f'{BASE_DIR}/haarcascade/haarcascade_frontalface_default.xml')
...
print(f"{len(files)} image data collected")

if not os.path.exists(f"{BASE_DIR}/model"):
    os.makedirs(f"{BASE_DIR}/model")

# labels.pickle store the dict of labels.
# {name: id}
# id starts from 0
with open(f"{BASE_DIR}/model/labels.pickle", "wb") as f:
    pickle.dump(label_ids, f)

# Train the recognizer and save the trained model.
recognizer.train(x_train, np.array(y_label))
recognizer.save(f"{BASE_DIR}/model/train.yml")
